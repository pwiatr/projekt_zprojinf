...
import com.targ.krakowski.web.rest.util.PaginationUtil;
...
public class ExtendedUserResource {
     ...
     * or with status 500 (Internal Server Error) if the extendedUser couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    ...
    @Timed
    public ResponseEntity<List<ExtendedUser>> getAllExtendedUsers(@ApiParam Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to get a page of ExtendedUsers");
        Page<ExtendedUser> page = extendedUserRepository.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/extended-users");
    ...
    @Timed
    public ResponseEntity<List<ExtendedUser>> searchExtendedUsers(@RequestParam String query, @ApiParam Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to search for a page of ExtendedUsers for query {}", query);
        Page<ExtendedUser> page = extendedUserSearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/extended-users");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
